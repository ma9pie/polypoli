...
function Profile() {
  ...
  // 정상적으로 입력했을 시 완료하기 버튼 활성화
  useEffect(() => {
    let check = true;
    if (userImg < 0 || imgList.length <= userImg) check = false;
  ...
  const changeProfileImage = (d) => {
    ...
    if (tmpImgNum < 0) {
      tmpImgNum = imgList.length - 1;
    } else if (tmpImgNum >= imgList.length) {
      tmpImgNum = 0;
    }
  ...
  const signupComplete = () => {
    ...
    const userRegionAndProfileData = {
      ...
      userArea: userState.userArea,
      userRegion: userState.userRegion,
      regionCongressmanId: userState.regionCongressmanId,
      ...userProfileData,
    };
  ...
  return (
    <Container>
      ...
      <ButtonBox>
        {activeBtn ? (
          <Button onClick={signupComplete}>완료하기</Button>
        ) : (
